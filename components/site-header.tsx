...
export function SiteHeader() {
  ...
  // Oculta o menu do topo nestas páginas para não distrair o usuário na conversão
  ...
  return (
    <>
      {/* ⚠️ AVISO ESTRATÉGICO NO TOPO (TOP BAR) ⚠️ */}
      <div className="w-full bg-gradient-to-r from-orange-600 to-amber-500 text-white text-xs sm:text-sm font-bold text-center py-2.5 px-4 relative z-[60] shadow-sm">
        <Link href="/gratis" className="hover:underline inline-flex items-center justify-center gap-2">
          🎁 Novidade: Liberamos uma Biblioteca de E-books 100% Grátis. Acesse Agora! <span aria-hidden="true">&rarr;</span>
        </Link>
      </div>
      ...
      <header className="w-full bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-[1400px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3 md:gap-6">
            ...
            {/* ⚠️ MENU BIBLIOTECA GRÁTIS ⚠️ */}
            <Link 
              href="/gratis" 
              className="text-sm font-bold text-orange-600 hover:text-orange-700 transition-colors hidden lg:block"
            >
              🎁 E-books Grátis
            </Link>
