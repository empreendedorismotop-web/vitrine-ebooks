...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const carregarDados = async () => {
      // 1. OTIMIZAÇÃO: Busca APENAS os VIPs para o Carrossel (Busca Seletiva)
      const { data: dataVips } = await supabase
        .from('profiles')
        .select('id, nome, titulo_ebook, imagem_url, favoritos_count, posicao_fixa') // <-- Traz apenas o necessário!
        .eq('status', 'ativo')
      ...
      const CACHE_KEY = '@vitrine-letreiro-cache'
      const TIME_KEY = '@vitrine-letreiro-time'
      const HORAS_CACHE = 2 // Define quantas horas a vitrine fica sem pedir dados novos ao Supabase
      ...
      if (dadosSalvos && !tempoExpirado) {
          ...
          // Busca TODOS os e-books ativos para o Letreiro Suave (Apenas ID, Titulo e Nome)
          const { data: dataAtivos } = await supabase
            .from('profiles')
            .select('id, titulo_ebook, nome') // <-- Traz apenas o necessário!
            .eq('status', 'ativo')
  ...
  return (
    <>
      ...
      <main className="overflow-hidden bg-slate-50 relative z-10">
        ...
        {vips.length > 0 && (
            <section className="max-w-7xl mx-auto px-4 md:px-6 mb-12">
               ...
               <div className="relative group">
                 ...
                 <div 
                    ref={carouselRef}
                    className="flex gap-4 sm:gap-6 overflow-x-auto pb-6 snap-x snap-mandatory [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden relative z-10"
                 >
                    {vips.map(vip => (
                        <div key={vip.id} className="snap-start shrink-0 w-[160px] sm:w-[280px] md:w-[320px] bg-white rounded-2xl shadow-sm border border-amber-200 overflow-hidden flex flex-col relative transition-all hover:shadow-md hover:border-amber-300">
                           ...
                           {/* Botão de Favorito - Reduzido no Mobile */}
                           <button 
                           ...
                           <div className="p-3 sm:p-5 flex flex-col flex-1 border-t border-slate-100">
                              ...
                              <div className="mt-auto">
                                 <Link href={`/ebook/${vip.id}?origem=Vitrine Principal (Carrossel VIP)`} className="block w-full text-center bg-slate-900 hover:bg-slate-800 text-white font-bold py-2 sm:py-3.5 rounded-lg sm:rounded-xl text-xs sm:text-sm transition-colors shadow-sm">
                                    Mais Detalhes
                                 </Link>
                              </div>
                           </div>
        ...
        {/* GRADE ORIGINAL DOS PRODUTOS (A grade principal que lista tudo) */}
